...
namespace crypto::sha256 {
    std::vector<uint8_t> hash(const std::vector<uint8_t>& input) {
        ...
        hash_functions obj = hash_functions();
        obj.stateregister();
        obj.adjust_digest(input.data(), input.size());
        obj.digest_final(digest);

        return std::vector<uint8_t>(digest, digest + hash_functions::PADD_SIZE);
    }

    std::string hexidigest(std::string input) {
        unsigned char digest[hash_functions::PADD_SIZE];
        memset(digest, 0, hash_functions::PADD_SIZE);
    ...
    std::string hexidigest(std::vector<uint8_t> data) {
        unsigned char digest[hash_functions::PADD_SIZE];
        memset(digest, 0, hash_functions::PADD_SIZE);

        hash_functions obj = hash_functions();
        obj.stateregister();
        obj.adjust_digest((unsigned char *)&data[0], data.size());
        obj.digest_final(digest);
